...
from db_connect import xgboost_USD, xgboost_USD_remove
...
def xgboost_dollar(conn, num, df):
    # 이거를 num만큼 반복
    for j in range(1, num+1):
        ...
        xgboost_USD(conn, today, dollar_close)
        xgboost_USD_remove(conn)
    return


if __name__ == "__main__":
    df_xg = pd.DataFrame()
    df_xg, df_notuse1, df_notuse2 = currency_craw()
    df_xg = df_xg.reset_index()
    conn = DBConnect()

    xgboost_dollar(conn, 30, df_xg)
